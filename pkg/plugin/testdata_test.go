...
func withRowValue[T string | int32 | bool | float64 | time.Time](value T) func(index int, n *data.Frame) {
...
func anOdataEntityType(name string, builders ...func(*odata.EntityType)) *odata.EntityType {
	et := &odata.EntityType{
		...
		NavigationProperties: []*odata.NavigationProperty{},
	}
	for _, build := range builders {
		build(et)
	}
...
func anOdataNavigationProperty(name string, propertyType string) *odata.NavigationProperty {
	return &odata.NavigationProperty{
		XMLName: xml.Name{Space: "https://docs.oasis-open.org/odata/ns/edm", Local: "NavigationProperty"},
		Name:    name,
		Type:    propertyType,
	}
}
...
func withNavigationProperty(name string, propertyType string) func(n *odata.EntityType) {
	return func(et *odata.EntityType) {
		et.NavigationProperties = append(et.NavigationProperties, anOdataNavigationProperty(name, propertyType))
	}
}
...
func withEntitySetResource(name string, entityType string) func(n *schema) {
	return func(resource *schema) {
		resource.EntitySets = append(resource.EntitySets, *anEntitySet(name, entityType))
	}
}
...
func withNavigationPropertyResource(name string, propertyType string) func(n *entityType) {
	return func(et *entityType) {
		et.NavigationProperties = append(et.NavigationProperties, navigationProperty{
			Name: name,
			Type: propertyType,
		})
	}
...
// --- $apply (groupby/aggregate) related ---
func withGroupBy(builders ...func(*property)) func(n *queryModel) {
	return func(model *queryModel) {
		for _, build := range builders {
			p := aProperty()
			build(&p)
			model.GroupBy = append(model.GroupBy, p)
		}
	}
}

func withAggregates(builders ...func(*aggregateSpec)) func(n *queryModel) {
	return func(model *queryModel) {
		for _, build := range builders {
			a := aggregateSpec{}
			build(&a)
			model.Aggregates = append(model.Aggregates, a)
		}
	}
}

func anAggregate(property func(*property), function string, alias string) func(*aggregateSpec) {
	return func(agg *aggregateSpec) {
		p := aProperty()
		property(&p)
		agg.Property = p
...
func decimalProp(p *property) {
	p.Name = "price"
	p.Type = odata.EdmDecimal
}
func dateProp(p *property) {
	p.Name = "invoice_date"
